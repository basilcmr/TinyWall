...
namespace pylorak.TinyWall
{
    internal sealed class TinyWallController : ApplicationContext
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.mnuModeJellyMode = new System.Windows.Forms.ToolStripMenuItem();
            this.mnuUpdateFromUpstream = new System.Windows.Forms.ToolStripMenuItem();
            ...
            // mnuModeJellyMode
            // 
            this.mnuModeJellyMode.Name = "mnuModeJellyMode";
            this.mnuModeJellyMode.Text = "Jelly Mode";
            this.mnuModeJellyMode.Click += new System.EventHandler(this.mnuModeJellyMode_Click);
            // 
            // mnuUpdateFromUpstream
            // 
            this.mnuUpdateFromUpstream.Name = "mnuUpdateFromUpstream";
            this.mnuUpdateFromUpstream.Text = "Update from Upstream...";
            this.mnuUpdateFromUpstream.Click += new System.EventHandler(this.mnuUpdateFromUpstream_Click);
            // 
            // mnuManage
            // 
        ...
        private void mnuModeJellyMode_Click(object sender, EventArgs e)
        {
            if (!EnsureUnlockedServer())
                return;

            SetMode(FirewallMode.JellyMode);
            UpdateDisplay();
        }

        private void mnuUpdateFromUpstream_Click(object sender, EventArgs e)
        {
            if (Utils.ShowMessageBox("This will fetch the latest official changes from upstream, merge our custom JellyMode, and rebuild the application. Do you want to continue?", "Update from Upstream", TaskDialogCommonButtons.Yes | TaskDialogCommonButtons.No, TaskDialogIcon.Warning) != DialogResult.Yes)
                return;

            try
            {
                string runDir = AppDomain.CurrentDomain.BaseDirectory;
                string batPath = Path.Combine(runDir, "UpdateAndBuild.bat");
                if (!File.Exists(batPath))
                {
                    string parent = Path.GetDirectoryName(runDir)!;
                    batPath = Path.Combine(parent, "UpdateAndBuild.bat");
                    if (!File.Exists(batPath))
                    {
                ...
                {
                    Utils.ShowMessageBox("UpdateAndBuild.bat was not found! Please ensure it is placed in your Jellyfin Net block root directory.", "Error", TaskDialogCommonButtons.Ok, TaskDialogIcon.Error);
                }
            }
            catch (Exception ex)
            {
                Utils.ShowMessageBox($"Failed to start update process: {ex.Message}", "Error", TaskDialogCommonButtons.Ok, TaskDialogIcon.Error);
            }
        }
